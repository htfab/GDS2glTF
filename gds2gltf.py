...
PDKS = {
    "sky130A": {
        "layerstack": layerstack_sky130A,
        "cell_name_property": 61,
    },
    "sg13g2": {
        "layerstack": layerstack_sg13g2,
        "cell_name_property": 1,
    },
}
DEFAULT_PDK = "sky130A"
...
def gds2gltf(
    ...
    pdk_config = PDKS.get(pdk_name, None)
    if pdk_config:
        layerstack = pdk_config["layerstack"]
        cell_name_property = pdk_config["cell_name_property"]
    else:
    ...
    for cell in layout.cells:  # loop through cells to read paths and polygons
        ...
        for layer_number, polygons in layers.items():
            ...
            for index, (polygon, _, _) in enumerate(polygons):
                ...
                num_triangles[layer_number] += (
                    num_polygon_points * 2 + len(triangles["triangles"]) * 2
                )
            ...
            for i, (_, poly_data, clockwise) in enumerate(polygons):
                ...
                p_indices_right = np.stack(
                    (
                        ind_list_bottom,
                        np.roll(ind_list_bottom, -1, axis=0),
                ...
                if len(gltf_indices) == 0:
                    ...
                    gltf_indices = np.append(
                        gltf_indices, p_indices + indices_offset, axis=0
                    )
                indices_offset += len(p_positions)

            indices_binary_blob = (
                gltf_indices.astype(np.uint32).flatten().tobytes()
            )  # triangles.flatten().tobytes()
            positions_binary_blob = gltf_positions.astype(
                np.float32
            ).tobytes()  # points.tobytes()
...
def main():
    parser = argparse.ArgumentParser(description="Convert GDSII to glTF")
    parser.add_argument("gds_file", type=str, help="Shuttle ID")
    parser.add_argument(
        "-p",
        "--pdk",
        type=str,
